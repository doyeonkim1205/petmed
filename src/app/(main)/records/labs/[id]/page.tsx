...
// 검사 기록 상세 — 수치 목록 + 추이 그래프 + 결과지 첨부(조회 전용) + 수정/삭제.
...
import { LAB_TEMPLATES, getAnalyte, analyteDisplay, templateLabel, type LabTemplateKey } from '@/lib/labCatalog';
...
export default function LabDetailPage() {
  ...
  useEffect(() => {
    if (!id) return;
    getLabTest(id).then((data) => { setTest(data); setLoading(false); trackEvent('lab_detail_view'); });
  }, [id, getLabTest]);
  ...
  useEffect(() => {
    const onPop = () => {
      if (typeof sessionStorage !== 'undefined' && sessionStorage.getItem('lab_updated_id') === id) {
        ...
        getLabTest(id).then((data) => { if (data) setTest(data); });
      }
    };
  ...
  const orderedCats = [...new Set([...(test.categories || []).filter((c) => c !== 'custom'), ...LAB_TEMPLATES.filter((x) => x.key !== 'custom').map((x) => x.key)])];
  const groups: { tpl: typeof LAB_TEMPLATES[number]; vals: typeof values }[] = [];
  for (const catKey of orderedCats) {
    const tpl = LAB_TEMPLATES.find((x) => x.key === catKey);
    if (!tpl) continue;
  ...
  const renderValueRow = (v: typeof values[number]) => {
    ...
    const lbl = meta ? analyteDisplay(meta, locale) : (v.label || v.analyte_key);
    ...
    <div className="bg-white min-h-full pb-24">
      <div className="sticky top-0 z-30 bg-white">
        <header className="relative flex items-center justify-center px-4 h-[60px]">
          ...
          <h1 className="text-sm font-semibold text-gray-700">{t('lab.detail.title')}</h1>
        </header>
      </div>
      ...
      <div className="max-w-sm mx-auto px-4 pt-4">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[15px] font-bold text-gray-900 truncate">{catSummary(test.categories, locale, t)}</p>
            <p className="text-[12px] text-gray-400">{test.test_date}{test.hospital_name ? ` · ${test.hospital_name}` : ''}</p>
          </div>
          <div className="flex items-center gap-0.5 flex-shrink-0">
            <button onClick={() => router.push(`/records/labs/${id}/edit?from=detail`)} className="p-1.5 text-gray-700 hover:text-gray-900 transition-colors" aria-label={t('common.edit')}><Pencil size={16} /></button>
            <button onClick={() => setConfirmDel(true)} disabled={deleting} className="p-1.5 text-red-400 hover:text-red-600 disabled:opacity-50 transition-colors" aria-label={t('common.delete')}><Trash2 size={16} /></button>
          </div>
        </div>
        ...
        {values.length > 0 ? (
          <div className="space-y-3">
            {groups.map(({ tpl, vals }) => (
              <div key={tpl.key}>
                <p className="text-[11px] font-bold text-gray-400 mb-1 px-0.5">{templateLabel(tpl, locale)}</p>
                    ...
                    <div key={v.id}>
                      ...
                      <div className="px-3 pt-1 pb-2">
                        <div className="relative w-fit max-w-full flex items-start gap-2 rounded-xl bg-white border border-blue-300 px-3 py-2 text-[11px] font-medium text-blue-600 shadow-[0_6px_22px_rgba(37,99,235,0.28)]">
                          ...
                          <p className="flex-1 leading-snug break-keep">{t('lab.detail.trendHint')}</p>
                          <button type="button" onClick={dismissTrendHint} aria-label={t('common.close')} className="-mt-0.5 -mr-0.5 flex-shrink-0 p-0.5 text-blue-400 hover:text-blue-600 active:scale-90"><X size={16} strokeWidth={2.5} /></button>
                        </div>
                      </div>
                    </div>
                  ) : renderValueRow(v))}
            ...
            {customVals.length > 0 && (
              <div>
                <p className="text-[11px] font-bold text-gray-400 mb-1 px-0.5">{t('lab.detail.custom')}</p>
                <div className="rounded-xl border border-gray-100 divide-y divide-gray-50">{customVals.map(renderValueRow)}</div>
              </div>
            )}
          </div>
        ) : (
          <p className="text-sm text-gray-400 py-8 text-center">{t('lab.detail.noValues')}</p>
        )}

        {test.memo && (
          <div className="mt-5">
            <p className="text-[11px] font-bold text-gray-400 mb-1">{t('lab.detail.memo')}</p>
        ...
        {files.length > 0 && (
          <div className="mt-5">
            <p className="text-[11px] font-bold text-gray-400 mb-1.5">{t('lab.detail.attachments')}</p>
            <div className="space-y-1.5">
              {files.map((f) => (
                <button key={f.id} onClick={() => openFile(f)}
                  ...
                  <span className="text-[10px] text-gray-300 flex-shrink-0">{f.file_type.startsWith('image/') ? t('lab.detail.photo') : t('lab.detail.pdf')}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <ConfirmModal
        open={confirmDel}
        title={t('lab.detail.deleteTitle')}
        message={t('lab.detail.deleteMsg')}
