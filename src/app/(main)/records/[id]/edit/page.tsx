...
export default function RecordEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const formState: RecordDraft = {
    ...
    recordColor, nextAppointmentColor, nextAppointmentReason, petId,
  };
  ...
  const loadData = async () => {
    try {
      ...
      setNextAppointmentReason(record.next_appointment_reason || '');
      ...
      setRecordOrigin({
        ...
        nextAppointmentReason: record.next_appointment_reason || '',
        petId: record.pet_id,
      });
    ...
    try {
      ...
      if (recordType === 'daily') {
        ...
        await updateRecord(id, {
          ...
          next_appointment_reason: recordType === 'visit' && nextAppointmentDate ? (nextAppointmentReason.trim() || null) : null,
              ...
              {nextAppointmentDate && (
                <div className="ml-3 pl-3 border-l-2 border-purple-200 space-y-2.5">
                  <div>
                    ...
                    <input type="text" value={nextAppointmentReason} onChange={(e) => setNextAppointmentReason(e.target.value)} maxLength={40}
