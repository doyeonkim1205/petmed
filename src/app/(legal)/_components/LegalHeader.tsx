...
export function LegalHeader({ title }: { title: string }) {
  ...
  return (
    <header className="relative flex items-center justify-center px-4 h-[60px] sticky top-0 z-10 bg-white dark:bg-gray-900" style={{ paddingTop: 'env(safe-area-inset-top)', height: 'calc(3.75rem + env(safe-area-inset-top))' }}>
      <button
        onClick={() => router.back()}
